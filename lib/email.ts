      ...
      html: `
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p style="color: #9ca3af; font-size: 12px; text-align: center; margin-top: 20px;">
            This is an automated email from CPECC
          </p>
        </body>
        </html>
      `,
...
export async function sendApprovalConfirmationEmail(user: {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${FROM_NAME}" <${fromEmail}>`,
      to: user.email,
      subject: 'Your Registration Has Been Approved - CPECC',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; margin-bottom: 20px;">
              Great news! Your registration has been approved. You now have full access to the CPECC.
            </p>
...
export async function sendRejectionEmail(user: {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${FROM_NAME}" <${fromEmail}>`,
      to: user.email,
      subject: 'Registration Update - CPECC',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
